...
// --- MOCK ACTIONS (FOR PREVIEW ONLY) ---
// const signIn = async (email, password) => {
//   console.log('Signing in with:', email, password);
//   // Simulate a delay and a fake check
//   await new Promise(resolve => setTimeout(resolve, 1000));
//   if (password === 'password') return { user: { email } }; // Success mock
//   return { error: 'Invalid email or password' }; // Failure mock
// };

// const signUp = async (email, password, name) => {
//   console.log('Signing up with:', email, password, name);
//   await new Promise(resolve => setTimeout(resolve, 1000));
//   return { user: { email, name } }; // Success mock
// };
// ---------------------------------------
...
export default function AuthForm() {
  ...
  const handleSubmit = async (event: any) => {
    ...
    const name = formData.get('fullName'); // Match the input name='fullName'
    ...
    try {
      if (isLogin) {
        ...
        // FIX 3: Pass extracted variables to signUp
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-gray-100 px-4 py-12">
      <div className="w-full max-w-md bg-white rounded-lg shadow-md p-6 sm:p-8">
        ...
        {error && (
          <div className="mb-4 p-3 text-sm text-red-800 bg-red-50 rounded-lg border border-red-200" role="alert">
        ...
        <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            <div className="space-y-4">
              <div className="flex items-start">
                <input
                  ...
                  type="checkbox"
                  className="w-4 h-4 border border-gray-300 rounded bg-gray-50 focus:ring-3 focus:ring-blue-300 mt-1"
                  required
                />
          ...
          <button
            type="submit"
            disabled={isLoading}
            className={`w-full text-white focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center ${isLoading ? 'bg-blue-400 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700'}`}
          >
